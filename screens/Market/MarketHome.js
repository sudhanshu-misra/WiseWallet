...
const MarketHome = ({navigation}) => {
  ...
  const handleAddToWishlist = async product => {
    ...
    try {
      ...
      const response = await axios.post(
        `${host.apiUrl}/api/wishlist/toggle-wishlist`,
        {
          productId,
        },
        config,
  ...
  const getWishlist = async () => {
    try {
      ...
      const response = await axios.get(
        `${host.apiUrl}/api/wishlist/get-user-wishlist`,
        config,
      );
                  ...
                  <View className="flex flex-col w-[60%] p-3 justify-between">
                    <View className="">
                      ...
                      {product.validityDate && (
                        <Text className="text-sm text-black">
                          Valid till {product.validityDate.slice(0, 10)}
                        </Text>
                      )}
                    </View>
                    {product.isSold ? (
                      <TouchableOpacity
                        onPress={() =>
                          handleNotificationWhenAvailable(product)
                        }>
